...
- **Automatic-triage replay is operator-only, exact-revision, and admission-safe.**
  ...
  The exact-selector gate also owns the distinct observation-drift targeted-refresh class proven by card #1584; `docs/AGENT_RUNTIME.md` owns its operator contract, while `triage_replay._observation_drift_refresh_refusal` is the single code owner of its proof.
