...
- `python tests/test_triage_replay.py` - fully offline operator-only replay coverage for exact-number card/source reads, strict exact-revision and identity gates, once-per-revision replay markers, terminal-error and absent-cache recovery, fail-closed malformed/mismatch matrices, dry-run zero-write behavior, reviewable wave bounds, and bounded dual-written triage result records.
