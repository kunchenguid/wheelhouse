...
That timeout is measured from JOB start while the pinned claude-code-action owns the model process, so `childExecutionTimeoutMs` in `agent_runtime/task_builder.py` carries the action's hard budget rounded up to whole minutes PLUS `CLAUDE_ACTION_JOB_OVERHEAD_MS` (2 minutes, mirroring the `+ 2` policy the `claude-model-call` composite action documents for the claude-cli lane) for the measured ~30-40s pre-model setup and post-model capture/upload.
Without the allowance the enforced model budget falls below the designed hard budget and the job timeout can kill the action before it commits its execution file - the card #1759 `consumer.committed.primary.output.missing` class (a terminal `triage_status: error` cache, recoverable only through the operator-run exact-card replay's ordinary terminal-error branch).
`tests/test_agent_runtime_child_timeout.py` pins the per-action values and the single-owner wiring.
