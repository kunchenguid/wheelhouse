...
- **`nl_decisions`** in `decision-handler.yml`: a plain-English owner/maintainer comment is
  ...
  Like `triage.yml`/`deep-review.yml`, the NL prompt is PASS-BY-REFERENCE (card
  #555 E2BIG fix): `nl-fetch` writes the bounded (1,500,000-byte cap, explicit
  truncation marker) target title/body/diff to on-disk `target.txt` and
  `apply_decision.build_nl_prompt` only NAMES it - target content is NEVER
  inlined into the Claude `prompt:`/`ALL_INPUTS`, so the prompt is
  PR-size-independent. `Read,Grep,Glob` are the tools that let the model open
  the file; the write/acting boundary is unchanged. If the LLM step then crashes
  so `route` is skipped, a final `always()` step posts ONE bounded, content-free,
  fire-once card note (marker `wheelhouse-nl-error:<comment-id>`, `github.token`)
  so the owner is not left with a silent red run - it changes no label, gate, or
  decision. See `tests/test_nl_prompt_size.py`.
...
- `python tests/test_nl_prompt_size.py` - the PASS-BY-REFERENCE NL decision prompt (card #555 E2BIG fix), offline: it runs the REAL `apply_decision.cmd_nl_prompt` with a synthetic on-disk `target.txt` at/above the #555 ~1.2 MB scale (and 5 MB), proving the diff never enters the emitted prompt, the prompt stays under a small fixed budget and far below `MAX_ARG_STRLEN` json-escaped, size is FLAT regardless of target size, and the OLD inline design WOULD have exceeded the limit; that `build_nl_prompt` no longer takes/inlines `target_content` and names `target.txt` with the Read tool + untrusted framing; the `nl-fetch` on-disk 1,500,000-byte cap with an EXPLICIT truncation marker (no silent cut, old unbounded `gh pr diff` gone); both NL Claude steps gaining `Read,Grep,Glob` while keeping their exact write/shell isolation (search = `Read,Grep,Glob,Write,Bash(wheelhouse-search)`, legacy = `Read,Grep,Glob,Write` with no shell/`GH_TOKEN`); and the fire-once NL-failure card note (fires only when `route` was skipped after a built prompt, `github.token`, marker-keyed idempotency, no prompt/comment content, no label/gate/decision change). `test_decision.py`/`test_nl_decisions_search.py` were updated for the new `build_nl_prompt(card_body, comment, kind, ...)` signature and the legacy step's `Read,Grep,Glob,Write` tools.
