...
- **Automatic-triage replay is operator-only, exact-revision, and admission-safe.**
  ...
  The same exact-selector gate also owns the distinct observation-drift targeted-refresh class (card #1584): a persisted ADMITTED assessment that lost currency purely because the card's review observation rotated on an UNCHANGED head can never heal by itself (the head-keyed triage cache stays fresh, so no scan re-triages, and the residual authority makes the advisory class refuse). `triage_replay._observation_drift_refresh_refusal` is the single code owner of that proof; an admitted plan clears the drifted cache (stale assessment, residual recommendation, verdict, primary/consumption telemetry), writes the version 1 marker with `cleared: observation-drift`, and re-enters the same spend-guarded queue path with the attempt cap preserved.
