...
- **Scan-time auto-merge (V1) is a STRICT SUBSET of the manual merge gate, built
  ...
  `render_card.normalize_automerge_verdict` is the semantic admission owner: it verifies exact evidence references from `target.txt` or the exact workspace/manifest-bound `target-src/`, matches restoration polarity after bounded contraction normalization, derives rather than trusts each typed assertion's subject/effect, reconciles every protected-contract clause across all claims and quotes without dropping coordinated subjects in either order, applies documentation/test object-topic neutrality consistently across protected categories, and projects the versioned `behavior_admission` record, while `auto_merge.behavior_verdict_facts` revalidates that record for both criteria display and acting.
