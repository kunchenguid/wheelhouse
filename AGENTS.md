...
- `python tests/test_option_b_path_completeness.py` - the high-volume Option B changed-path boundary: a 226-candidate repository still performs one bounded immutable base/head comparison per observed PR independently of DecisionContext bounds; the shared evaluator/reducer read, digest/count/head binding, complete-repository control, and truncated/absent/read-failure fail-safe unknown projection are covered offline.
