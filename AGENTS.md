...
- **Automatic-triage spend guards are enforced inside the shared queued checkpoint.**
  ...
  The global-only `triage_daily_ceiling` defaults to 100 reservations per UTC day, accepts integers 1 through 2000, and fails closed to 0 when invalid; one reservation authorizes one triage run whose bounded schema repair permits at most two model calls.
