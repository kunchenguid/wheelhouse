...
- **Scan-time auto-merge (V1) is a STRICT SUBSET of the manual merge gate, built
  ...
  `render_card.normalize_automerge_verdict` is the semantic admission owner: it verifies exact evidence references from `target.txt` or the exact workspace/manifest-bound `target-src/`, uniquely matches each restoration predicate/argument polarity after canonical bounded contraction normalization, derives rather than trusts each typed assertion's subject/effect from one authoritative protected-pattern table, uniquely maps protected occurrences, unions pending coordinated subjects, binds their effects inside atomic clauses, applies only locally governed documentation/test object-topic neutrality across every protected category, and projects the versioned `behavior_admission` record, while `auto_merge.behavior_verdict_facts` revalidates that record for both criteria display and acting.
