...
- **Automatic-triage replay is operator-only, exact-revision, and admission-safe.**
  ...
  The evidence-empty E7 recovery has one incident-scoped attempts reset in `scripts/triage_replay.py`.
  It is admitted only for the dedicated sanctioned wave plus the explicitly supplied complete 19-card cohort, and every card is bound to its exact diagnosed revision and prior replay marker before its trusted count of 2 is reset to 1 for the queued write.
  The reset writes a one-use v2 replay marker, refuses policy drift away from cap 2, and leaves both the global per-revision cap and daily ceiling unchanged.
...
- `python tests/test_triage_replay.py` - fully offline operator-only replay coverage for exact-number card/source reads, strict exact-revision and identity gates, claim tombstone verification, duplicate-only re-entry, terminal-error and absent-cache recovery, admission-duplicate card projection, fail-closed malformed/mismatch matrices, dry-run zero-write behavior, reviewable wave bounds, bounded dual-written triage result records, and the exact-cohort one-use evidence-empty attempts reset with the global cap pinned at 2.
