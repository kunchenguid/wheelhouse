...
- **Held cards - a card is not owner-visible in its normal form until its
  ...
  **Fail-open safety net for a `triage.yml` run that never reaches its
  ...
  `render_card.py`. Its inline cache-clear fallback is therefore limited to
  issue-triage cards. A PR-review card is left untouched until trusted card
  maintenance can atomically update both its triage block and criteria.
...
- **`nl_decisions`** in `decision-handler.yml`: a plain-English owner/maintainer comment is
  ...
  Missing, multiple, or invalid native output fails closed into the one bounded `nl-decision.schema-repair` turn, which deliberately stays on the portable `trusted-post-action-bridge` mechanism for the pinned CLI's pre-2.1.205 schema-ignore hazard.
