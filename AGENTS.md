...
- `python tests/test_config_schema.py` - structural load test that `wheelhouse_core.load_config()` accepts the checked-in `wheelhouse.config.yml`: every `repos:` entry is well-formed (name matches its key; `compliance_check` is null or a non-empty string; `test_check_patterns` is a list of non-empty strings; `merge_method` is unset or squash|merge|rebase) and repo names are unique. Deliberately pins no repo names or fleet size, so it keeps guarding the file as the fleet grows/shrinks, no network.
