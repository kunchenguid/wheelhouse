...
- **Credential-free public evidence is advisory, never authority.** Only a trusted, current, complete projection may inform triage or existing auto-merge G6 facts; every mutation still requires a fresh exact owner-comment `AuthorityDecision`. The authoritative design, security boundary, and acceptance paths live in `docs/AGENT_RUNTIME.md` under "Credential-free public evidence."
