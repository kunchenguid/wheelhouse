...
- **Automatic-triage replay is operator-only, exact-revision, and admission-safe.**
  ...
  `docs/AGENT_RUNTIME.md` owns the claim tombstone, duplicate-only evidence, and exact-revision admission-denial contracts.
...
- **Scan-time auto-merge (V1) is a STRICT SUBSET of the manual merge gate, built
  ...
  Every PR-review triage body mutation other than the no-trusted-source security fallback (queued, deferred, cleared, failed, or completed) goes through `render_card._atomic_automerge_card_body`: it applies the triage state first, calls the code-owned `evaluate_candidate` exactly once with that candidate body as `card_entry` under the fleet read token, projects the visible and frozen criteria from that result, restores the default card token, and performs one card-body write. The fallback runs only when the trusted source is unavailable, so it cannot load code or perform fleet reads; it clears the queued cache under the default card token and renders a visible `### Triage` security-fallback warning that any temporarily stale criteria reflect the prior queued state until trusted card maintenance resumes.
...
One direct supervisor step serves the direct actions; schema repair remains one-turn/no-tool, while public advisory review uses only the canonical typed public-read MCP tools.
...
The direct lane pins `ubuntu-24.04`, installs and verifies the exact Bubblewrap package from `runtime.lock.json`, proves a minimal namespace before provider admission, verifies Claude CLI `2.1.215`, passes the OAuth token only through a private file into the Claude child environment, and uses the existing Bubblewrap supervisor with native structured output; schema repair exposes zero tools, while public advisory review exposes only its typed broker tools.
...
- **Credential-free public evidence is advisory, never authority.** The authoritative implementation is `agent_runtime/public_read.py`, `agent_runtime/brokers.py`, `agent_runtime/exercise.py`, `agent_runtime/vision_policy.py`, and `agent_runtime/mcp_bridge.py`; production/adversarial acceptance is `tests/test_public_read_broker.py` plus both public jobs in the `Agent Runtime canary`. The public broker has no repo/domain allowlist for genuinely public HTTPS, but is a separately sandboxed, credential-free, resolve-then-pin process with bounded typed operations and immutable receipts. `exercise.run` is a separate no-network process with only reviewed adapters and fixed scenarios. Generic policy comes only from the target default-branch `VISION.md`, and unknown or ambiguous normative language blocks a positive verdict. Public-evidence output is always `AdvisoryReview`, never `AuthorityDecision`; raw evidence cannot act. Under pragmatic Option B, only a trusted, current, complete projection may inform triage and existing auto-merge G6 facts. Every mutation still requires a fresh exact owner-comment `AuthorityDecision`, with closed conditional facts evaluated deterministically against the current projection.
...
- `python tests/test_public_read_broker.py` - real credential-free broker/MCP/public-TLS test plus generic two-fixture VISION and authority-separation checks; `--production-e2e` runs the exact Bubblewrap broker against a local TLS adversary, SSRF/rebinding redirects, and a real depth-1 public Git snapshot.
