...
- **Display-only migration for cards the observation-bound writer cannot reach.**
  ...
  `projection_writer.py`, so `_edit_issue_body` refuses one and `upsert_card`
  DEFERS a refresh whose current observation is incomplete
  (`defer card refresh ...: current PR observation is unavailable`). That
  whole-card guard is correct and must stay. Its side effect is that a card
  parked in `ci-state-unknown` silently misses every `CARD_RENDER_VERSION`
  display migration while it stays unobservable - the five `no-mistakes` cards
  the 11 -> 12 canonical-recommendation migration left behind had already missed
  8 -> 9, 9 -> 10, and 10 -> 11. `scripts/presentation_migration.py` is the ONE
  bounded mechanism that closes that gap: an operator supplies an exact cohort
  (`--cards N,N`, max 25), every member is preflighted (open machine-created
  pure `needs-decision` pr-review card whose recorded head still equals the live
  target head), and ONE ambiguous member denies the whole run before any write.
  Dry-run is the default; `--apply` is required to write. The pure transform,
  the structural diff allowlist, the invariant check, and the single verified
  writer live in `render_card.py`
  (`presentation_migration_body`, `presentation_removable_lines`,
  `presentation_diff_allowed`, `presentation_migration_verify`,
  `edit_presentation_only_body`) beside the authoritative-writer rule they are
  the sole exception to. Load-bearing invariants: deletions only, and only of
  lines structurally derived from the retired surfaces in the original body; the
  hidden state block must be byte-identical; `render_version` is NEVER advanced,
  so the card stays render-stale and a later complete observation still performs
  the ordinary full renderer migration; title, labels, options, target, and
  model cache are never written; idempotent by content. Covered by
  `tests/test_presentation_migration.py` against the five exact production
  bodies in `tests/fixtures/frozen_observation_cards.json`.
...
- `python tests/test_presentation_migration.py` - the bounded display-only migration for incomplete-observation cards, no network: the five exact frozen production bodies are pinned as fixtures; the transform is deletions-only against a structural allowlist, keeps the hidden state block byte-identical, never advances `render_version` (so the card stays render-stale for the real migration), preserves every section marker and decision option, and is idempotent; verification fails closed on added, modified, non-allowlisted-removed, render-version-advancing, and authority-injecting bodies, and the verified writer refuses an unchecked body; dry-run writes nothing; apply migrates the cohort and a repeat run is a `noop`; one moved target head, in-flight decision, human-authored card, or closed card denies the entire run before any write; a non-pr-review or already-clean card is never rewritten; the cohort selector is bounded and fails closed; and the module can reach no label, title, target, or fleet-token path while the whole-card observation guard and authoritative-writer rule stay intact.
