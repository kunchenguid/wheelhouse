...
- **`triage.yml` - automatic, lightweight, advisory PR-card OR issue-card context.** Triggered by `scan-backstop` / `reconcile.py` and the ingest fast path for pure `needs-decision` pr-review OR issue-triage cards whose current revision (a PR's `head_sha`, or an issue's `updated_at`) does not match `triaged_sha`; if the card is held under `pending-triage`, the update path also publishes its real checkboxes fail-open.
  ...
  **Pass-by-reference prompt (do not reinline).** The prompt is deliberately SMALL and PR-SIZE-INDEPENDENT: the runner pre-fetches the title/body/diff (at the verified revision) into the on-disk `target.txt`, checks out the code at `target-src/`, and the constant prompt only NAMES those files and directs the agent to Read/Grep/Glob them - it never `cat`s target content into the `prompt:` input. This structurally kills the card #517 E2BIG class (a diff inlined into the prompt inflated `claude-code-action`'s `ALL_INPUTS` env var past the kernel's `MAX_ARG_STRLEN=131072` per-string `execve` limit, so bash could not spawn and no triage ran); `data/wheelhouse-card517-d3` / `wheelhouse-triagearch-v2` carry the root cause and design. The on-disk diff cap is generous (`diff_limit_bytes=1500000`, bounding the model's context when it Reads, NOT the prompt), and `DIFF_COMPLETE` now means "the whole diff is on disk (not truncated by that cap, not binary/LFS/submodule)", so large-but-sub-cap PRs now get the auto-merge behavior verdict they were silently denied under the old 120000 embed cap; a diff exceeding even the generous cap is truncated and fails closed (`DIFF_COMPLETE=false`, no verdict). The `<target-content>` delimiters ride inside `target.txt`, and the small this-repo card body may still be inlined in deep-review (bounded by card size, not PR size). The required `evidence` field (verbatim quotes the model copies from what it read) is the lazy/fabrication guard: `normalize_triage` rejects a result missing it (fail-closed to the no-result path), and `triage-apply` additionally anchor-checks the quotes against the on-disk `target.txt` via `--target-file` (`evidence_anchor_ok`; fail-OPEN if `target.txt` is unreadable so a checker hiccup never rejects a genuine triage). The no-`READONLY_TOKEN` path still works because the pre-fetched local files ARE the whole context; `wheelhouse-search` is an optional freshness/cross-reference add-on only.
...
- **`deep-review.yml` - ALWAYS-ON, code-grounded (no enable flag).** Triggered by ticking the **Investigate** box on a card, by the repo owner applying the `needs-deep-review` label, or by the repo owner running `workflow_dispatch` with only `issue=...` for direct verification.
  ...
  It uses the SAME pass-by-reference prompt shape as `triage.yml` (see there): the title/body/diff is written to the on-disk `target.txt` (with the `<target-content>` delimiters) and the prompt only NAMES `target.txt` + `target-src/` for the agent to Read - it never inlines target content into the `prompt:` input. This was the WORSE E2BIG offender: it used to embed an UNCAPPED `gh pr diff`, so it would fail to spawn on any large PR the moment Investigate was clicked; the diff is now bounded on disk (`diff_limit_bytes=1500000`) exactly like triage. The small this-repo card body stays inlined (bounded by card size).
