...
- `python tests/test_nl_schema_repair.py` - card #1433 bounded NL schema repair, no network: the malformed `\`-before-backtick regression through the trusted Claude bridge; delivered-only trigger discipline; bounded, tokenless, one-turn, no-tool repair; unchanged `nl-decision-v1` schema binding; success-on-repair routing to a real answer; still-invalid repair denial with precise retryable projection; and static workflow proof that neither runtime branch can recurse into another repair attempt.
