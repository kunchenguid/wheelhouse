...
- **Credential-free public evidence is advisory-only.** The authoritative implementation is `agent_runtime/public_read.py`, `agent_runtime/brokers.py`, `agent_runtime/vision_policy.py`, and `agent_runtime/mcp_bridge.py`; production/adversarial acceptance is `tests/test_public_read_broker.py` plus the `Agent Runtime canary`. The broker has no repo/domain allowlist for genuinely public HTTPS, but is a separately sandboxed, credential-free, resolve-then-pin process with bounded typed operations and immutable receipts. Generic policy comes only from the target default-branch `VISION.md`. Public-evidence output is always `AdvisoryReview`, never `AuthorityDecision`; `apply_decision.py` and auto-merge G6 reject it as authority. Only an exact current owner-comment binding may authorize an NL mutation.
