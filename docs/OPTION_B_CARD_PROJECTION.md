...
A supported `decision:*` label erased between the writer's reread and PATCH has one narrow recovery path in `decision_label_recovery.py`. It requires the already-authorized webhook actor, exact card repository and node identity, identical target/head/observation/context binding, the exact label-set delta, and a complete label-event sequence consisting only of the triggering human `labeled` event followed by the trusted projection `unlabeled` event. A verified bot-authored claim comment makes that event single-use. The workflow rereads the claim and event sequence after taking `processing` and immediately before target action. Old, duplicate, replayed, new-head, relabeled, explicitly removed, ambiguous, malformed, or unsupported label events remain inert.
