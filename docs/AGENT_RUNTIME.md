...
The advisory transport schema is draft-07 because pinned Claude Code 2.1.215 rejects the former draft-2020-12 schema before model initialization. The trusted supervisor independently validates the model-derived plan, coverage audit, every receipt, and every VISION obligation, then converts raw model output to an `AdvisoryReview` projection with no action fields and `acting_authority:false`. Raw evidence and raw model output never act. Under the captain's pragmatic Option B, only a trusted, current, complete projection may contribute triage facts and satisfy the existing auto-merge G6 behavior facts; stale, unavailable, ambiguous, injected, disputed, or incomplete evidence fails closed. `apply_decision.py` always rejects `AdvisoryReview` as a decision. Only `AuthorityDecision`, bound to the exact latest owner comment ID and SHA-256 body and independently checked for an explicitly named mutation, can reach deterministic execution. Conditional owner commands use a closed evidence-blind fact schema against the current trusted projection, and the executor revalidates the target, base, and VISION revisions before every conditional mutation.
