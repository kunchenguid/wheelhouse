...
After a successful supersede write, replay also polls the same `render_card.get_card` path the projection CAS uses until that exact tombstone (comment id + superseded marker) is visible, so GitHub read-after-write lag cannot make consecutive card snapshots disagree about the replay's own comment edit.
...
Triage normalization and the trusted card consumer share one compact-object extractor and one `target-anchor/v1` implementation. After JSON decoding, that anchor parser recognizes straight single- and double-quoted spans. Within a span, an odd run of backslashes before the matching delimiter removes only its final escape slash; all preceding slashes, mismatched quote characters, other Unicode characters, and source text remain significant. Even slash runs remain literal, malformed or mismatched delimiters cannot fall back to unquoted anchoring, and the resulting span must still occur verbatim in the immutable target after the existing case, whitespace, backtick, and asterisk normalization. Provider prose around one schema-valid compact triage object is transport metadata, not a schema failure. A VISION evidence list may be empty when no declared criterion matches, but every supplied criterion remains strictly schema-validated and downstream VISION/source binding still fails closed. Optional `source_provenance` is emitted only after a sanctioned public clone returns a non-empty resolved commit and inspected-file observations; unavailable or failed required inspection omits the object and remains conservatively inconclusive rather than emitting an invalid empty stub. A structurally valid candidate with unanchored evidence is `output.evidence_invalid`, never `output.schema_invalid` or success. A genuinely schema-invalid but delivered triage candidate remains available to its one-turn repair policy.

The PR-triage recommendation basis is a discriminated union with exactly three kinds: `other`, `configured-tests-not-run`, and `configured-tests-not-green`. `other` omits `check_names`, including when green configured checks support the rationale; the two negative configured-test kinds require at least one named check. Trusted assessment admission canonicalizes the schema's absent `other.check_names` to the existing empty-list representation; a present non-empty list remains denied. Configured-test bases with an empty list remain admissible at this boundary as a pre-existing behavior, while the schema rejects that shape on the ordinary generation path; the schema-repair restore path validates through `normalize_basis`, so this residual gap remains reachable and is tracked separately pending measurement.

The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings, each with its own exact `{source, quote}` reference. Every evidence `source` is a workspace path: use exactly `target.txt`, or `target-src/<repository-relative-path>` for a checked-out target file (for example, `target-src/tests/fm-composer-lib.test.sh`), never a bare repository-relative path such as `tests/fm-composer-lib.test.sh`. Only a VISION-derived behavior assertion may use exactly `vision.md`; class-B restoration remains restricted to `target.txt` or `target-src/<repository-relative-path>`. The trusted consumer verifies references against `target.txt`, the exact trusted `vision.md` only for behavior assertions, or the declared file below `target-src/`, parses each bounded restoration proposition into a canonical predicate plus complete ordered active, progressive, or supported compound-passive argument-role spans from an explicit positive grammar and predicate-local polarity after contraction normalization, and requires both claims and quotes to carry the same complete canonical affected-object role under a bounded defect-to-repair predicate relation before admitting class B. Lexical scanning consumes every character of the normalized proposition exactly once, retains bounded numeric and symbolic identifiers as ordered semantic tokens, treats only terminal sentence punctuation as neutral, and rejects unsupported or ambiguous spans. Bounded `@identifiers` survive semantic cleanup, evidence matching, admission, and persisted state unchanged; card presentation removes the `@` only when rendering visible model text so it cannot notify an account. Progressive `-ing` predicates preserve active roles; every temporal or relational complement, other subordinate proposition, extra proposition, residual complement, unsupported role phrase, unsupported material between an auxiliary and predicate, unsupported passive chain, and morphologically ambiguous passive chain remains unavailable. In-job reads use the exact workspace checkout; cross-job reads use a bounded regular-file bundle whose manifest binds every path, size, digest, and total to the exact checked-out revision. A bounded `behavior_assertions` array explicitly types the governed subject and effect of every protected-contract clause across the summary, product implications, top-level evidence, restoration claims, restoration quotes, and assertion quotes. Trusted normalization uses one authoritative protected-pattern table and a deterministic span representation to map every protected occurrence uniquely, union and consume pending coordinated subjects, retain and consume every protected semantic effect exactly once within atomic clauses, including possessive, pronominal, deictic, and referential references, and require the complete claim/subject/effect set to match, so model-supplied labels never authorize behavior. Changed, tightened, or newly required protected contracts, every unmatched default phrase outside a proven documentation/test topic except a fully consumed isolated new opt-in disabled-by-default construction with no protected subject or behavior-change effect when the declared class is C, and behavior objects are denied, while restoration and unchanged defaults are explicitly bound and locally governed documentation/test object-topic effects, including pre-parsed coordinated protected topic edges in either word order, and bounded negative modal/compound-passive no-change forms remain neutral. Reverse topic neutrality supports fully consumed active, compound-passive, or modal-passive documentation-edit predicates with no residual agent, object, or clause; synthetic unrelated effects require a fully parsed independent non-governance object rather than absence from a reference blacklist. Mixed behavior predicates use per-occurrence binding or remain unavailable. Unmapped occurrences, ambiguous scope, unconsumed effects, and verbs merely named `test` after any protected category cannot acquire documentation neutrality. The consumer records one versioned `behavior_admission`; missing, malformed, older, generic, incidentally overlapping, polarity-inverted, role-reversed, unrelated, source-unbound, unreconciled, ambiguous, or contradictory records remain unavailable or unmet at the shared display and acting boundary. The direct no-tool PR schema-repair output intentionally excludes `automerge`, `vision_evidence`, and `source_provenance`; those acting claims cannot be repaired without source tools and therefore fail closed to unavailable G6. Trusted consumer code may restore only an already-valid exact `recommendation_basis` from the original candidate before normal observation/context admission. The repair model cannot invent or alter that binding or restoration evidence.

`AgentResult.status` and normalized output events describe trusted transport, schema, and evidence validation. The later `consumer.committed` stage describes a separately bound card projection, whose triage status and behavior verdict remain independent. When a failed primary result still has a delivered candidate that advisory normalization consumes, the card keeps `triage_status=succeeded` for compatibility but also records `triage_primary_status=failed`, the bounded primary error code, and `triage_consumption=advisory`, with visible warning copy; this never grants authority. Each layer emits one terminal result for its own execution ID or event key rather than overwriting the other layer's evidence.
