...
The native PR-triage schema requires an `other` recommendation basis to carry an empty `check_names` array. Named checks are valid only for `configured-tests-not-run` or `configured-tests-not-green`; malformed combinations are rejected during generation and remain denied by trusted assessment admission.
