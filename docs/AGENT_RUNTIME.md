...
The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings. The trusted consumer requires both statements to be concrete, share a multi-token domain subject, and bind each claim to its own exact anchor-verified evidence span carrying that subject before admitting class B. It records a versioned `behavior_admission` and deterministically evaluates each change predicate and governed object to reject affirmative contract-change or new-requirement claims in the same summary, product implications, or evidence, while excluding explicit no-change and documentation/test-only objects. Missing, malformed, older, generic, incidentally overlapping, unrelated, source-unbound, or contradictory admission records remain unavailable or unmet at the shared display and acting boundary. Schema repair must preserve an existing `automerge` object unchanged and cannot invent restoration evidence.
