...
The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings, each with its own exact `{source, quote}` reference. The trusted consumer verifies references against either `target.txt` or the declared file below `target-src/`, requires every meaningful claim token in its corresponding quote, and requires both claims and quotes to share a multi-token domain subject before admitting class B. A bounded `behavior_assertions` array explicitly types the governed subject and effect of every summary, product-implication, or evidence clause that mentions a protected contract; the exact claim set must reconcile, and every assertion carries a verified reference. Changed, tightened, or newly required protected contracts are denied, while typed documentation/test effects and unchanged contracts remain neutral. The consumer records one versioned `behavior_admission`; missing, malformed, older, generic, incidentally overlapping, unrelated, source-unbound, unreconciled, or contradictory records remain unavailable or unmet at the shared display and acting boundary. Schema repair must preserve an existing `automerge` object unchanged and cannot invent restoration evidence.
