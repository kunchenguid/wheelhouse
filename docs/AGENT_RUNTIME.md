...
The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings. The trusted consumer requires that pair before admitting class B, records a versioned `behavior_admission`, and deterministically rejects affirmative contract-change claims in the same summary, product implications, or evidence. Missing, malformed, older, or contradictory admission records remain unavailable or unmet at the shared display and acting boundary. Schema repair must preserve an existing `automerge` object unchanged and cannot invent restoration evidence.
