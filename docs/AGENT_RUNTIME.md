...
The native PR-triage schema uses a discriminated recommendation-basis union with one strict four-field wire shape: an `other` basis requires an empty `check_names` list, while `configured-tests-not-run` and `configured-tests-not-green` require at least one named check. The schema expresses this contract directly, and trusted assessment admission remains strict.
