...
The two schema-repair actions and the three credential-free public advisory pipeline actions resolve to the exact-pin direct Claude CLI profile through the guarded production activation.
...
The direct lane pins the model job to `ubuntu-24.04`, installs and verifies Bubblewrap `0.9.0-1ubuntu0.1`, exercises a real namespace before provider admission, verifies Claude CLI `2.1.215` against the platform digest in `runtime.lock.json`, binds the OAuth credential through one mode-0600 file, and launches the existing supervisor and worker inside the Bubblewrap provider-only sandbox. Bubblewrap is launched through the runner's passwordless `sudo` solely because Ubuntu 24.04 denies unprivileged loopback setup in a new network namespace; it clears the environment and retains only the setup capabilities needed to hand off to the runner UID/GID before executing the model process. The sandbox explicitly creates the read-only `/etc` mount parents and a mode-1777 isolated `/tmp`, so the unprivileged worker can read its runtime mounts and write only to its isolated temporary and output mounts. After exit, the trusted supervisor restricts any output-ownership normalization to regular expected result and diagnostic files before validating them.
...
`agent_runtime/adapters/claude.py` implements the direct Claude CLI boundary used by both schema-repair actions and the three public advisory pipeline actions.
...
`advisory-review.public` is a distinct, authority-free result path for PR triage when the target default branch has a readable `VISION.md` and the complete PR diff is available. Trusted task construction copies that exact VISION and deterministically binds stable units with exact text, locations, and hashes. An isolated structured PolicyDeriver pass receives only those pinned units and produces a generic EvidencePlan. An independent structured CoverageAuditor pass receives only the same VISION and proposed plan and checks every unit, normative condition, requiredness decision, and omission before target or public evidence is considered. No project, package, or domain policy vocabulary is encoded in the runtime. Trusted validation requires exact unit identity, allowlisted operations, complete classification, current VISION binding, and exact deriver/auditor agreement. A missing or unreadable VISION, disagreement, omitted obligation, unavailable receipt, truncated required source, or unknown or ambiguous clause prevents a positive projected verdict.
...
The broker is a separate process launched by `PublicReadBrokerProcess` with a curated non-secret environment, empty home, private PID/mount/IPC/UTS namespaces, no credential or provider sockets, and only its public network route plus one typed Unix socket. Bubblewrap setup may use the hosted runner's passwordless namespace privilege, but drops to the runner UID/GID before Python starts. Both broker sandboxes explicitly create their `/etc` mount parent and a mode-1777 isolated `/tmp` before that handoff, preserving runner access without leaving root-owned output. The launcher never recursively changes ownership, broadens a shared parent mode, or sends a root signal to a negative process-group ID. It receives a dedicated four-file read-only runtime staging directory, never the runner workspace or the broader agent runtime tree. Admission validates stable private directory identities, and the attestation records the process chain, complete allowed environment-name inventory, mount list, empty home, and inaccessible credential paths. The broker/process canary exercises this production Bubblewrap artifact against a real local TLS adversary, public Git snapshot, released artifact exercise, SSRF fixtures, and both generic VISION fixtures with no secrets.

The advisory transport schema is draft-07 because pinned Claude Code 2.1.215 rejects the former draft-2020-12 schema before model initialization. The trusted supervisor independently validates the model-derived plan, coverage audit, every receipt, and every VISION obligation, then converts raw model output to an `AdvisoryReview` projection with no action fields and `acting_authority:false`. Raw public evidence and `AdvisoryReview` cannot authorize, initiate, bypass, or replace auto-merge or any mutation. Under the captain's pragmatic Option B, only a trusted, current, complete projection may contribute triage facts and supply validated advisory facts to the already-existing deterministic G6 eligibility gates, which remain independently enforced. Stale, unavailable, ambiguous, injected, disputed, or incomplete projections supply no facts. `apply_decision.py` always rejects `AdvisoryReview` as a decision. Only `AuthorityDecision`, bound to the exact latest owner comment ID and SHA-256 body and independently checked for an explicitly named mutation, can reach deterministic execution. Conditional owner commands use a closed evidence-blind fact schema against the current trusted projection, and owner-command mutations still require fresh genuine owner authority while the executor revalidates the target, base, and VISION revisions before every conditional mutation.
...
The `Agent Runtime canary` separates three claims: a credential-free real Bubblewrap broker/process adversarial E2E, a model-free exact installer and sandbox probe, and a mandatory fresh real-model product E2E. The product job uses Claude Code `2.1.215`, the exact production draft-07 advisory schema, production task builder, supervisor, typed public/exercise brokers, AXI and unrelated VISION fixtures, and preserved receipt evidence. The workflow is green only when all three jobs pass.
