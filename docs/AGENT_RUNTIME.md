...
The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings. The trusted consumer requires both statements to be concrete, share a specific domain subject, and link that subject to the exact-source evidence before admitting class B. It records a versioned `behavior_admission` and deterministically rejects affirmative contract-change or new-requirement claims in the same summary, product implications, or evidence, while excluding explicit no-change and documentation/test-only clauses. Missing, malformed, older, generic, unrelated, source-unbound, or contradictory admission records remain unavailable or unmet at the shared display and acting boundary. Schema repair must preserve an existing `automerge` object unchanged and cannot invent restoration evidence.
