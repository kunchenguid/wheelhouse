...
The PR-triage output schema bounds the optional `class_b_restoration` object to separate corrected-defect and intended-behavior-restored strings, each with its own exact `{source, quote}` reference. The trusted consumer verifies references against either `target.txt` or the declared file below `target-src/`, requires every meaningful claim token and its explicit polarity after bounded contraction normalization to match the corresponding quote, and requires both claims and quotes to share a multi-token domain subject before admitting class B. In-job reads use the exact workspace checkout; cross-job reads use a bounded regular-file bundle whose manifest binds every path, size, digest, and total to the exact checked-out revision. A bounded `behavior_assertions` array explicitly types the governed subject and effect of every protected-contract clause across the summary, product implications, top-level evidence, restoration claims, restoration quotes, and assertion quotes. Trusted normalization derives those semantics with a finite conservative grammar, preserves every coordinated protected subject in either order, and requires the complete claim/subject/effect set to match, so model-supplied labels never authorize behavior. Changed, tightened, or newly required protected contracts are denied, while documentation/test object-topic effects are neutral for every protected category and bounded negative modal/passive no-change forms remain neutral. The consumer records one versioned `behavior_admission`; missing, malformed, older, generic, incidentally overlapping, polarity-inverted, unrelated, source-unbound, unreconciled, ambiguous, or contradictory records remain unavailable or unmet at the shared display and acting boundary. Schema repair must preserve an existing `automerge` object unchanged and cannot invent restoration evidence.
