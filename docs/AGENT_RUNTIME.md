...
The native PR-triage schema uses a discriminated recommendation-basis union: an `other` basis omits `check_names`, while `configured-tests-not-run` and `configured-tests-not-green` require at least one named check. The schema expresses this contract directly, and trusted assessment admission remains strict.
