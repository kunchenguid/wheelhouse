...
The PR-triage recommendation basis is a discriminated union: `other` omits `check_names`, while configured-test variants require at least one named check. Trusted assessment admission canonicalizes the schema's absent `other.check_names` to the existing empty-list representation; a present non-empty list remains denied. This admits the schema-valid spelling as equivalent to the prior internal empty-list form without expanding which basis claims can authorize a recommendation.
