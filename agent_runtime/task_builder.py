...
def build_task(
    *,
    ...
    tool_instruction = (
        ...
        "mcp__wheelhouse__exercise_run. First read exactly /work/vision.md, "
        "/work/vision-units.json, /work/policy-binding.json, /work/policy-derivation.json, "
        "/work/policy-audit.json, and /work/target.txt with mcp__wheelhouse__fs_read. "
        "Call each requested public-evidence operation exactly once with the prompt's exact "
        "arguments. Do not retry an unavailable result. For each policy.assess obligation, use "
        "the bound target facts to return pass or fail with no citation, or not_applicable only "
        "when its explicit applicability condition is false. For each public operation, return "
        "pass with the matching complete receipt citation when it establishes the requirement. "
    ...
    shim_lines = [
        ...
        *(
            [
                ...
                "inspection. Naming a reference implementation does not itself require source "
                "inspection: a structural-similarity criterion still maps only to policy.assess. "
                "Do not add a public operation alongside policy.assess for the same local judgment. "
                "Headings are recognized context-only units with no obligations. Never "
