...
def _require_bound_clean_repository(repo: Path, commit: str) -> str:
    ...
    # AgentTask git.detached is a const true claim: require a true detached HEAD, not a branch tip.
    abbrev = _git_text(repo, "rev-parse", "--abbrev-ref", "HEAD").strip()
    if abbrev != "HEAD":
        raise ArtifactError("repository HEAD must be detached at the bound commit")
    # Fail closed on any dirty, staged, untracked, intent-to-add, or mode-mismatch path.
