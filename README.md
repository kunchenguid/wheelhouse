...
> Class B also requires bounded corrected-defect and restored-behavior claims, each with uniquely matched predicate/argument polarity after canonical bounded contraction normalization, its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference, and a shared concrete subject. Trusted normalization maps every protected occurrence from one pattern table, unions pending coordinated subjects, binds effects inside atomic clauses, treats locally governed documentation/test object topics consistently across protected categories, and requires complete reconciliation; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
