...
> Class B also requires bounded corrected-defect and restored-behavior claims, each bound to its own exact `target.txt` or `target-src/` evidence reference and a shared concrete subject. Typed behavior assertions must reconcile every protected-contract clause in the result; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
