...
> Class B also requires bounded corrected-defect and restored-behavior claims, each polarity-matched after bounded contraction normalization to its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference and a shared concrete subject. Trusted normalization derives the subject/effect semantics of every protected-contract statement across all declared claims and quotes, preserving coordinated subjects in either order, treating documentation/test object topics consistently, and requiring complete reconciliation; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
