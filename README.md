...
The current production selection uses the exact pinned Claude Action for eight actions and the pinned direct Claude CLI for both schema-repair actions plus credential-free public advisory review, all inside the same separate read-only reusable model workflow from a bounded content-addressed task handoff.
