...
> Class B also requires bounded corrected-defect and restored-behavior claims, each carrying a uniquely matched canonical active, progressive, or explicitly supported compound-passive predicate, complete ordered argument-role spans, predicate-local polarity, no temporal or relational complement, extra proposition, or residual subordinate/complement tokens after contraction normalization, its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference, and the same complete canonical affected-object role under a bounded defect-to-repair predicate relation. Trusted normalization maps every protected occurrence from one pattern table, unions pending coordinated subjects, retains and consumes protected semantic effects exactly once inside atomic clauses, fails closed on every unmatched default phrase outside a proven documentation/test topic except a fully consumed isolated new opt-in disabled-by-default class-C construction with no protected subject or behavior-change effect, explicitly binds behavior objects, accepts unrelated effects only for fully parsed independent non-governance objects, and preserves coordinated documentation/test topic edges in either word order only when one fully consumed active, compound-passive, or modal-passive documentation-edit predicate owns every topic; changed, tightened, or newly required contracts and behaviors are denied, while bounded negated modal and compound-passive documentation edits remain unchanged. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
