...
- **Auto triage (default-on, opt-out)** - when a PR-review card is created or refreshed to a new head, Claude does a quick read-only pass and writes Summary, Product implications, and a Recommended next step into the card.
  ...
  Public evidence always renders as an authority-free advisory. A trusted, current, complete projection may inform triage and the existing auto-merge G6 behavior facts, but it cannot create an Accept recommendation action, authorize a merge, or authorize any other write. A positive advisory also fails closed if any VISION-derived required check or complete receipt is missing.
