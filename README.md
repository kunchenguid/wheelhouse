...
> Class B also requires bounded evidence naming the corrected defect and intended behavior restored. Trusted admission rejects a verdict when its own summary, product implications, or evidence says an existing mode, default, workflow, or delivery contract is tightened or changed. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
...
- **Auto triage (default-on, opt-out)** - when a PR-review card is created or refreshed to a new head, Claude does a quick read-only pass and writes Summary, Product implications, and a Recommended next step into the card.
  ...
  Class B must include bounded corrected-defect and restored-behavior evidence. Trusted normalization records a fail-closed semantic admission result and rejects contradictions in the result's own summary, product implications, or evidence; missing historical admission data stays unavailable.
