...
- **Auto triage (default-on, opt-out)** - when a PR-review card is created or refreshed to a new head, Claude does a quick read-only pass and writes Summary, Product implications, and a Recommended next step into the card.
  ...
  Public evidence always renders as an authority-free advisory. It cannot create an Accept recommendation action, satisfy auto-merge, or authorize any write. A positive advisory also fails closed if any VISION-derived required check or complete receipt is missing.
