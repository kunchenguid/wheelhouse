...
> Class B also requires bounded corrected-defect and restored-behavior claims, each carrying a uniquely matched canonical active, progressive, or explicitly supported compound-passive predicate, argument roles, predicate-local polarity, no temporal complement, and no residual subordinate or complement tokens after contraction normalization, its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference, and a shared concrete subject. Trusted normalization maps every protected occurrence from one pattern table, unions pending coordinated subjects, retains and consumes protected semantic effects exactly once inside atomic clauses, explicitly binds defaults and behavior objects, accepts unrelated effects only for fully parsed independent non-governance objects, and preserves coordinated documentation/test topic edges in either word order only when one fully consumed active, compound-passive, or modal-passive documentation-edit predicate owns every topic; changed, tightened, or newly required contracts and behaviors are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
