...
> Class B also requires bounded corrected-defect and restored-behavior claims, each polarity-matched after canonical bounded contraction normalization to its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference and a shared concrete subject. Trusted normalization maps every protected occurrence from one pattern table, binds effects inside atomic coordinated clauses, treats locally governed documentation/test object topics consistently, and requires complete reconciliation; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
