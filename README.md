...
The current production selection uses the exact pinned Claude Action for the standard agent paths and the pinned direct Claude CLI for schema repair plus the credential-free public advisory pipeline, all inside the same separate read-only reusable model workflow from a bounded content-addressed task handoff. The exact action-to-profile inventory lives in [Agent runtime operations](docs/AGENT_RUNTIME.md#current-operating-state).
...
The pinned Claude Action remains the explicit production selection for the standard agent paths and the deployable schema-repair rollback target; schema repair and the credential-free public advisory pipeline use the direct Claude CLI through the same trusted boundary. See [Agent runtime operations](docs/AGENT_RUNTIME.md#current-operating-state) for the authoritative action-to-profile inventory.
...
- **Auto triage (default-on, opt-out)** - when a PR-review card is created or refreshed to a new head, Claude does a quick read-only pass and writes Summary, Product implications, and a Recommended next step into the card.
  ...
  For a complete-diff PR with a readable default-branch VISION, generic VISION review may use anonymous public HTTPS and depth-1 Git evidence through a separate credential-free broker. There is no repository or domain allowlist for genuinely public sources. The model receives only typed bounded operations, and every result is explicitly untrusted and backed by an immutable source URL, pinned IP, fetch-time, content-hash, and bounds receipt.
  Public evidence always renders as an authority-free advisory. Raw public evidence and `AdvisoryReview` cannot authorize, initiate, bypass, or replace auto-merge or any mutation. Only a trusted, current, complete projection may inform triage and supply validated advisory facts to the already-existing deterministic G6 eligibility gates, which remain independently enforced. Incomplete, stale, or unavailable projections supply no facts, and owner-command mutations still require fresh genuine owner authority. A positive advisory also fails closed if any VISION-derived required check or complete receipt is missing.
...
- **Plain English - just reply (opt-in).** When you turn on `nl_decisions` (see [step 4](#4-optional-add-the-claude-production-token-for-agent-assisted-features)), reply to a card in normal language and Claude maps what you meant onto the same actions above.
  ...
  - **Acts** when you're clearly deciding - "merge it", "close this, it's superseded by #50", "decline because the approach is wrong", or (pr-review only) "request changes, the tests are missing".
    ...
    You can make an action conditional on the current trusted public advisory, for example "merge it if the advisory is positive". Wheelhouse evaluates only the advisory's closed trusted facts, never raw public evidence, and rechecks the target, base, and VISION revisions before acting.
...
- **Token scope.** The default `GITHUB_TOKEN` only reaches this repo and is used for all card activity (so it can't recursively re-trigger the handler).
  ...
  Anonymous public-evidence review does not use `READONLY_TOKEN`, `FLEET_TOKEN`, the model OAuth token, or the job token in its broker process.
