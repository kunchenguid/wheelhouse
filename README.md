...
> Class B also requires bounded corrected-defect and restored-behavior claims, each bound to its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference and a shared concrete subject. Trusted normalization derives the subject/effect semantics of every typed protected-contract assertion and requires complete reconciliation; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
