...
> Class B also requires bounded corrected-defect and restored-behavior claims, each carrying a uniquely matched canonical active, progressive, or explicitly supported compound-passive predicate, complete ordered argument-role spans, predicate-local polarity, no extra proposition or residual subordinate/complement tokens after contraction normalization, its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference, and the same complete affected object under a bounded defect-to-repair predicate relation. Fully consumed temporal object spans remain exact and ordered. Trusted normalization maps every protected occurrence from one pattern table, unions pending coordinated subjects, retains and consumes protected semantic effects exactly once inside atomic clauses, fails closed on every unmatched default phrase outside a proven documentation/test topic, explicitly binds behavior objects, accepts unrelated effects only for fully parsed independent non-governance objects, and preserves coordinated documentation/test topic edges in either word order only when one fully consumed active, compound-passive, or modal-passive documentation-edit predicate owns every topic; changed, tightened, or newly required contracts and behaviors are denied, while bounded negated modal and compound-passive documentation edits remain unchanged. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
