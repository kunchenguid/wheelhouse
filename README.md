...
> Class B also requires bounded corrected-defect and restored-behavior claims, each with uniquely matched canonical active or explicitly supported compound-passive predicate, argument roles, and local polarity after contraction normalization, its own exact `target.txt` or exact-revision, manifest-verified `target-src/` evidence reference, and a shared concrete subject. Trusted normalization maps every protected occurrence from one pattern table, unions pending coordinated subjects, consumes protected semantic effects exactly once inside atomic clauses without anaphoric waivers, preserves coordinated documentation/test topic edges in either word order, and requires complete reconciliation; changed, tightened, or newly required contracts are denied. Older or incomplete verdicts without the current semantic admission record remain `UNAVAILABLE`, never eligible.
