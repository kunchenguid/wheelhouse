...
class VisionPolicyBoundaryTests(unittest.TestCase):
    def setUp(self):
        ...
        document = vision_unit_document(self.vision.read_text(encoding="utf-8"))
        unit = {
            **document["units"][0],
    ...
    def build(self, action, name, **kwargs):
        ...
        return build_task(
            ...
            revision="a" * 40, wheelhouse_revision="b" * 40,
    ...
    def test_three_tasks_have_distinct_input_boundaries_and_schemas(self):
        ...
        self.assertEqual({row["id"] for row in derive["spec"]["inputs"]}, {"vision", "vision-units"})
        self.assertEqual({row["id"] for row in audit["spec"]["inputs"]}, {"vision", "vision-units", "policy-derivation"})
        self.assertEqual(
            {row["id"] for row in advisory["spec"]["inputs"]},
            {"vision", "vision-units", "policy-derivation", "policy-audit", "target"},
        )
    ...
    def test_all_context_optionalization_and_disagreement_fail_closed(self):
        ...
        with self.assertRaises(VisionPolicyError):
            validate_policy_artifacts(self.vision.read_text(), context_plan, self.audit)
        ...
        _, _, complete = validate_policy_artifacts(self.vision.read_text(), self.plan, disagreement)
