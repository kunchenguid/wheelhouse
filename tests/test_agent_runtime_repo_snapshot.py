...
def main() -> None:
    with tempfile.TemporaryDirectory() as directory:
        ...
        # --- denial: branch-attached HEAD (detached:true must be truthful) ---
        attached = root / "attached"
        init_repo(attached)
        write_file(attached, "a.txt", "1\n")
        attached_commit = commit_all(attached, detach=False)
        # Confirm we are on a branch tip, not detached.
        abbrev = run_git(attached, "rev-parse", "--abbrev-ref", "HEAD").stdout.decode().strip()
        check("deny: fixture is branch-attached", abbrev != "HEAD")
        check(
            "deny: branch-attached HEAD rejected",
            rejects(lambda: snapshot_repository(attached, attached_commit), "detached"),
        )
